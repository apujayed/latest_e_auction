...
if(catalog !== undefined){
    (async()=>{
        ...
        let f_c = [];
        catalog.map((content)=>{
            if(content.Sale_number === parseInt(e_record[0].Sale_Number) && content.Season === e_record[0].Season){
                f_c.push(content)
            }
        })
        set_filtered_catalog(f_c)
    })()
}
    },[catalog])
    return (
            ...
            <div className="shadow-md max-w-md lg:max-w-2xl bg-white rounded-lg h-fit">
                <div className="py-3 px-6 border-b border-dashed">
                    <div className="flex justify-between items-center">
                        <h4 className="text-lg font-semibold tracking-tight text-slate-900">All lot</h4>
